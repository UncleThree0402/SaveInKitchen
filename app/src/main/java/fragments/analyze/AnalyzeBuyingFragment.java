...
public class AnalyzeBuyingFragment extends Fragment {
    ...
    private void setAverageTextView(long date) {
        mBuyHistoryViewModel.getBuyHistory30DaysAverage(date).observe(getViewLifecycleOwner(), new Observer<Double>() {
    ...
    private void setMostExpensiveValueTextViewTextView(long date) {
        mBuyHistoryViewModel.getBuyHistoryMostExpensive(date).observe(getViewLifecycleOwner(), new Observer<BuyHistory>() {
            @Override
            public void onChanged(BuyHistory buyHistory) {
                if (buyHistory == null) {
                    mMostExpensiveValueTextView.setText("Nothing");
                } else {
                    String text = buyHistory.getName() + " ( " + NumberFormatter.moneyFormatter(buyHistory.getCost()) + " ) ";
